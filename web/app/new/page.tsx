...
export default function NewPactPage() {
  ...
  // Humanized Validation (No all-caps error codes)
  let submitDisabled = false
  let submitReason = ''

  if (!amountMaker || makerAmountBn === 0n) {
    submitDisabled = true; submitReason = 'Enter an amount'
  } else if (!hasEnoughBalance) {
    submitDisabled = true; submitReason = `Not enough ${selectedTokenLabel}`
  } else if (terms.length < 20) {
    submitDisabled = true; submitReason = `Agreement terms must be at least 20 characters (${terms.length}/20)`
  } else if (!deadlineMinutes || Number(deadlineMinutes) < 2) {
    submitDisabled = true; submitReason = 'Deadline must be at least 2 minutes in the future'
  } else if (kind === 1 && (!amountTaker || amountTakerParsed() === 0n)) {
    submitDisabled = true; submitReason = 'Enter counterparty swap amount'
  }
          ...
          {createdPactId && (
            <div className="bg-[#0c0d10] border border-[#202126] p-4 rounded-md mb-6 text-left space-y-2.5">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] font-mono text-emerald-400">Includes Verified Terms Hash</span>
              </div>
  ...
  return (
    <main className="min-h-screen max-w-[680px] mx-auto pt-6 sm:pt-8 px-3.5 sm:px-6 pb-24 overflow-x-hidden">
      <Navbar />
      <TrustStrip />

      {/* Wrong Network Treatment: Clean Rose Inline Banner + Switch Button */}
      {isWrongChain && (
        <div className="bg-rose-500/10 border border-rose-500/30 rounded-md p-3.5 mb-6 text-xs font-mono flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-rose-300">
          <span className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-rose-400 animate-pulse" />
            <span>Unsupported network. Switch to Circle Arc Testnet to deploy.</span>
          </span>
          <button
            onClick={() => switchChain({ chainId: TARGET_CHAIN_ID })}
            className="bg-rose-500 text-black px-3.5 py-1.5 rounded text-xs font-bold hover:bg-rose-400 transition-colors shrink-0"
          >
      ...
      <div className="bg-[#141518] border border-[#24262d] rounded-md p-3.5 mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-sm">
        ...
        <button
          ...
          className="inline-flex items-center justify-center gap-1 bg-[#1e2026] hover:bg-[#282a32] text-emerald-400 border border-emerald-500/30 px-3 py-1.5 rounded text-xs font-mono font-medium transition-colors cursor-pointer shrink-0"
        >
      ...
      {address && makerBalance === 0n && (
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-md p-3.5 mb-6 text-xs font-mono flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 text-amber-300">
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="bg-amber-400 hover:bg-amber-300 text-black px-3 py-1 rounded text-xs font-bold tracking-wider inline-flex items-center justify-center gap-1 transition-colors shrink-0"
          >
            Get Free Test USDC ↗
          </a>
        </div>
      ...
      <div className="space-y-6">
        ...
        {/* Action Buttons with Humanized Theater */}
        <div className="pt-2">
          {submitDisabled ? (
            <button
              disabled
              className="w-full bg-[#141518] border border-[#202126] text-zinc-500 py-3 rounded-md font-mono text-xs font-medium cursor-not-allowed"
            >
              {submitReason}
            </button>
            ...
            <div className="w-full bg-[#111215] border border-amber-500/40 text-amber-400 py-3 rounded-md font-mono text-xs font-semibold text-center flex flex-col items-center justify-center gap-1.5">
              <div className="flex items-center gap-2">
                ...
                <span>Confirm token approval in your wallet…</span>
              </div>
              {approveTxHash && (
                <a
            ...
            <div className="space-y-2">
              <div className="text-[11px] text-center text-zinc-400 font-mono">
                Approve {selectedTokenLabel} so the pact can lock it (step 1 of 2)
              </div>
              <button
